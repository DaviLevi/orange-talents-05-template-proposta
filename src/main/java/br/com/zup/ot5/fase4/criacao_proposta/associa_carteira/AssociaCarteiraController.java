...
public class AssociaCarteiraController {
	...
	public ResponseEntity<?> associaCartaoAoPaypal(@PathVariable(required = false) String idCartao, 
			@RequestBody @Valid AssociaCarteiraPaypalRequest requisicao) {
		
		return associaCarteira(idCartao, requisicao, TipoCarteiraLegadoRequest.PAYPAL);
	}

	
	private ResponseEntity<?> associaCarteira(String idCartao, AssociaCarteiraRequest requisicao, TipoCarteiraLegadoRequest tipoCarteiraRequisitada) {
		if(idCartao == null) {
